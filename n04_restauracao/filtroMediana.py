...
def passagemFiltro(img, filtro, n):
    imgAux = img
    for i in range(n):
        imgAux = filtroMediana(imgAux, filtro)
...
ap.add_argument("-l", "--lmage", required=True, help="caminho para a image")
...
#plotagem do histograma resultante
plt.hist(imgR.ravel(),256,[0,256])
plt.title('Histograma Imagem Resultante')
plt.show()

# Histograma do ruido
ruido = plt.hist((img2.ravel() - img.ravel()),256,[0,256])
plt.title('Histograma Ruido')
plt.show()

#salvar imagem
salvar = "resultados/(" + str(filtro) + "-Mediana)" + args["image"][7:]
