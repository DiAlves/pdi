...
clc,clear
% abre a imagem
Img_Original = double(imread('image_3.png'));

BW_Original =  (im2bw(Img_Original));  % must set object region as 1, background region as 0 


changing = 1;
[rows, columns] = size(BW_Original);
BW_Thinned = double(BW_Original);
BW_Del = double(ones(rows, columns));
while changing
