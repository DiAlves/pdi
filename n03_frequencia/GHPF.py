...
def GHPF(img, n):
    ...
    salvar = "resultados/" + "(F - Antes - GHPF)" + args["image"][7:] 
    ...
    for i in range(p):
        for j in range(q):
            ...
            h[i, j] = 1 - np.exp(-(d**2)/(2*(n**2)))
                
    salvar = "resultados/" + "(Filtro-GHPF)" + args["image"][7:] 
    cv2.imwrite(salvar, 255*h)
    
    #Convolução no dominio da frequência
    g = imgFShift * h
    salvar = "resultados/" + "(F - Depois - GHPF)" + args["image"][7:] 
...
imgResult = GHPF(img, n)

salvar = "resultados/" + "(GHPF(" + str(n) + "))" + args["image"][7:] 
